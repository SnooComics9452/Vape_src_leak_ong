...
main(int argc, char* argv[])
{
    ...
    printf("nom cracked this bs - thank you very much kangaroo team couldnt have done it without u");
    Sleep(6000);
    ...
    process_memory = VirtualAllocEx(pi.hProcess, NULL, strlen("ripvape.dll"), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    if (!process_memory)
        return -1;

    if (!WriteProcessMemory(pi.hProcess, process_memory, "ripvape.dll", strlen("ripvape.dll"), NULL))
        return -1;
